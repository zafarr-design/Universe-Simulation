...
#define FANN_GET(type, name) \
  FANN_EXTERNAL type FANN_API fann_get_##name(struct fann *ann) { return ann->name; }

#define FANN_SET(type, name) \
  FANN_EXTERNAL void FANN_API fann_set_##name(struct fann *ann, type value) { ann->name = value; }
